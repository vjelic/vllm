...
#pragma unroll
  for (int i_stage = 0; i_stage < reduce_stage; i_stage++) {
    ...
    int32_t v_remote_tmp = __builtin_amdgcn_ds_bpermute(
        src_lane << 2, __builtin_bit_cast(int32_t, v_local));
...
__device__ void blockReduceMax(float& val, int& idx) {
  ...
  if (wid == 0) {
    val = (lane < (blockDim.x + WARP_SIZE - 1) / WARP_SIZE) ? shared_vals[lane]
                                                            : -INFINITY;
    idx = (lane < (blockDim.x + WARP_SIZE - 1) / WARP_SIZE) ? shared_idxs[lane]
                                                            : -1;
...
__global__ void grouped_topk_kernel(
    ...
    const size_t stride_tk, const int num_experts, const int topk,
    const int topk_group, const int num_tokens,
    const float routed_scaling_factor, const int num_fused_shared_experts) {
  ...
  const int topk_excluding_share_expert_fusion =
  ...
  if constexpr (!isSoftmax) {
    for (int e = threadIdx.x; e < num_experts; e += blockDim.x) {
      float gating =
          static_cast<float>(gating_output[token_idx * num_experts + e]);
...
#pragma unroll
    ...
    __syncthreads();
    thread_sum =
        wave_reduce(thread_sum, [](float a, float b) { return a + b; });
    for (int e = threadIdx.x; e < num_experts; e += blockDim.x) {
      scores[e] /= thread_sum;
    }
    __syncthreads();
...
#pragma unroll
  ...
  float sum = 0.0f;
  for (int k = 0; k < topk_excluding_share_expert_fusion; ++k) {
    float max_val = -INFINITY;
    int max_idx = -1;
...
#pragma unroll
  ...
  if (threadIdx.x == 0 && num_fused_shared_experts > 0) {
    int64_t last_idx = topk_excluding_share_expert_fusion;
    int64_t expert_offset = 0;
    topk_values[last_idx] = sum / routed_scaling_factor;
    topk_indices[last_idx] = static_cast<int32_t>(num_experts + expert_offset);
  }
  __syncthreads();

  if (need_renorm) {
    if (threadIdx.x == 0) {
      scores[0] = 1.0 / sum;  // reuse lds
    }
    __syncthreads();
    sum = scores[0];
  } else {
...
#define LAUNCHER4(VEC_F, NUM_GRP, need_renorm)                                \
  if constexpr (isBiased) {                                                   \
    LAUNCHER_biased_grouped_topk_kernel(VEC_F, NUM_GRP, need_renorm, true,    \
                                        false)                                \
  } else {                                                                    \
    if (isSoftmax) {                                                          \
...
#define LAUNCHER_biased_grouped_topk_kernel(VEC_F, NUM_GRP, need_renorm,  \
                                            isBiased, isSoftmax)          \
  VLLM_DISPATCH_FLOATING_TYPES(                                           \
      gating_output.scalar_type(), "biased_grouped_topk_kernel", [&] {    \
        aiter::grouped_topk_kernel<scalar_t, VEC_F, NUM_GRP, need_renorm, \
                                   isBiased, isSoftmax>                   \
            <<<grid, block, shared_mem_size, stream>>>(                   \
                gating_output.data_ptr<scalar_t>(),                       \
                correction_bias.data_ptr<scalar_t>(),                     \
                topk_weights.data_ptr<float>(), topk_ids.data_ptr<int>(), \
                stride_tk, num_experts, topk, topk_grp, num_tokens,       \
                routed_scaling_factor, num_fused_shared_experts);         \
      });

#define LAUNCHER_grouped_topk_kernel(VEC_F, NUM_GRP, need_renorm, isBiased, \
                                     isSoftmax)                             \
  VLLM_DISPATCH_FLOATING_TYPES(                                             \
      gating_output.scalar_type(), "grouped_topk_kernel", [&] {             \
        aiter::grouped_topk_kernel<scalar_t, VEC_F, NUM_GRP, need_renorm,   \
                                   isBiased, isSoftmax>                     \
            <<<grid, block, shared_mem_size, stream>>>(                     \
                gating_output.data_ptr<scalar_t>(), nullptr,                \
                topk_weights.data_ptr<float>(), topk_ids.data_ptr<int>(),   \
                stride_tk, num_experts, topk, topk_grp, num_tokens,         \
                routed_scaling_factor, num_fused_shared_experts);           \
      });
...
void biased_grouped_topk(
  ...
  size_t shared_mem_size =
      (num_experts * sizeof(float) + num_expert_group * sizeof(float) +
       num_expert_group * sizeof(bool) + topk * sizeof(int) +
...
void grouped_topk(torch::Tensor& gating_output,  // [num_tokens, num_experts]
                  ...
                  const double routed_scaling_factor = 1.,
                  const int64_t num_fused_shared_experts = 0) {
  TORCH_CHECK((scoring_func == "softmax") || (scoring_func == "sigmoid"),
              "grouped_topk scoring_func only support softmax or sigmoid");
