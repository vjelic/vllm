...
__device__ __forceinline__ floatx4 gcn_mfma16x16x8_xf32_instr(const floatx2 inpA,
                                                  const floatx2 inpB,
                                                  const floatx4& inpC) {
    return __builtin_amdgcn_mfma_f32_16x16x8_xf32(inpA, inpB, inpC, absz, cbid, blgp);
}
...
__global__ __launch_bounds__(NUM_THREADS) void paged_attention_ll4mi_QKV_mfma16_kernel(
  ...
  //__shared__ _B16x4 shared_logits[NWARPS][8][16][4];
  __shared__ floatx4 shared_logits[NWARPS][4][16][4];
  ...
  //floatx4 Qlocal[QKHELOOP][QK_SIZE_RATIO][2]; //note that 16 contiguous elements of Q should be fetched per lane for 8 bit cache types : QK_SIZE_RATIO changes for this
  ...
  //output layout from QKmfma : QH16xT4x4 16 qheads across 16 lanes, 16 tokens across 4 rowsx4 tokens per lane
    ...
    if (local_qhead_idx < GQA_RATIO) {
        ...
        if constexpr (KV_DTYPE == vllm::Fp8KVCacheDataType::kAuto) {
            ...
            for (int i=0; i<2; i++) {
                floatx4 frag;
                for(int j=0; j<4; j++) {
                    frag[j] = to_float_b16<scalar_t>(tmp.xy[i][j]);
                }
                shared_logits[offset1][lane4id][local_qhead_idx][i] = frag;
            }
        } else {
            for (int i=0; i<2; i++) {
                ...
                shared_logits[offset1][offset2][local_qhead_idx][offset3] = frag;
            }
        }
    } 
    __syncthreads();
    for (int qkhe_depth = 0; qkhe_depth < QKHELOOP; qkhe_depth++) {
        for (int qkratio = 0; qkratio < QK_SIZE_RATIO; qkratio++) {
            for (int i=0; i<2; i++) {
                ...
                Qlocal[qkhe_depth][qkratio][i] = shared_logits[qkhe_depth][rowid][lane16id % GQA_RATIO][2*qkratio + i];
            }
        }
    }
#endif
    ...
    constexpr int VBLOCKS_PER_LANE = VTOKENS_PER_LANE / BLOCK_SIZE;
        ...
        if constexpr (KV_DTYPE == vllm::Fp8KVCacheDataType::kAuto) {
            ...
            for (int qkratio = 0; qkratio < QK_SIZE_RATIO; qkratio++) {
              for (int i=0; i<2; i++) {
                ...
                _B16x4 Afragment = Klocal[token_depth][qkhe_depth].xy[i];
                ...
                for (int j=0; j<4; j++) {
                    Afragmentf32[j] = to_float_b16<scalar_t>(Afragment[j]);
                    Bfragmentf32[j] = to_float_b16<scalar_t>(Bfragment[j]);
                }
               
                for (int k=0; k<2; k++) {
                    floatx2 Afrag,Bfrag;
                    Afrag[0] = Afragmentf32[2*k];
                    Afrag[1] = Afragmentf32[2*k+1];
                    Bfrag[0] = Bfragmentf32[2*k];
                    Bfrag[1] = Bfragmentf32[2*k+1];
                    dout[token_depth] = gcn_mfma16x16x8_xf32_instr<0,0,0>(Afrag, Bfrag, dout[token_depth]);
                }
                //dout[token_depth] = gcn_mfma16x16x16_instr<scalar_t, 0, 0, 0>(Afragment, Bfragment, dout[token_depth]);
              }
            }
    ...
    for (int token_depth = 0; token_depth < TLOOP; token_depth++) {
        ...
        shared_logits[warpid][token_depth][lane16id][rowid] = dout[token_depth];
    }
        ...
        if constexpr (KV_DTYPE == vllm::Fp8KVCacheDataType::kAuto) {
          ...
          for (int vfetch_depth = 0; vfetch_depth < VTLANELOOP; vfetch_depth++) {
              for (int i=0; i<2; i++) {
                ...
                _B16x4 Afragment = Vlocal[vtoken_depth][vhe_depth][vfetch_depth].xy[i];
                //_B16x4 Bfragment = shared_logits[vtoken_depth][offset2][lane16id][offset1];
                floatx4 Afragmentf32, Bfragmentf32;
                ...
                for (int k=0; k<2; k++) {
                    ...
                    Bfrag[0] = Bfragmentf32[2*k];
                    Bfrag[1] = Bfragmentf32[2*k+1];
                    tmp_out = gcn_mfma16x16x8_xf32_instr<0,0,0>(Afrag, Bfrag, tmp_out);
                }
                //tmp_out = gcn_mfma16x16x16_instr<scalar_t, 0, 0, 0>(Afragment, Bfragment, tmp_out);
#endif
              }
          }
    ...
    if (warpid == 0) {
        ...
        for (int h = 0; h < GQA_RATIO4; h++) {
            ...
            for (int i=0; i<2; i++) {
                ...
                vout[h].xy[i] = from_floatx4<scalar_t>(shared_logits[offset1][offset2][local_head_idx][offset3+i]);
            }
        }
    ...
    if (lane16id < GQA_RATIO) {
        ...
        for (int vhe_depth = 0; vhe_depth < VHELOOP; vhe_depth++) {
            ...
            *out_ptr_B16x4 = from_floatx4<scalar_t>(outelems[vhe_depth]);
        }
    }
#endif
#if 0 //DEBUG ONLY 
