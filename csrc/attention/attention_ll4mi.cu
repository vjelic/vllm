...
template <typename scalar_t, int BLOCK_SIZE, int HEAD_SIZE, int BLOCK_RATIO_PER_WG, int NUM_THREADS, int GQA_RATIO>
__global__ __launch_bounds__(NUM_THREADS) void Attn_QK_new_kernel(  
  ...
  int max_ctx_blocks
      ...
      __shared__ float shared_qk_max[NWARPS][8+1];
      __shared__ float shared_exp_sum[NWARPS][8+1];
      constexpr int QHLOOP = 2; // each 4 lanes fetch 4 different qheads, total qheads =8, so qhloop is 2
      ...
      for (int h=0; h<QHLOOP; h++) {
          const int qhead_idx = h*4 + (laneid%4); 
...
template<
  ...
  int NUM_THREADS,
  int PARTITION_SIZE>
__global__ __launch_bounds__(NUM_THREADS) void paged_attention_v2_reduce_kernel_new(
  scalar_t* __restrict__ out,             // [num_seqs, num_heads, head_size]
...
#define LAUNCH_CUSTOM_ATTENTION                                                  \
  Attn_QK_new_kernel<T,BLOCK_SIZE,HEAD_SIZE,BLOCK_RATIO_PER_WG,NTHR,GQA_RATIO>      \
  <<<grid, block, 0, stream>>>(                                                 \
    query_ptr,                                                                                \
    key_cache_ptr,                                                                            \
    value_cache_ptr,                                                                          \
    ...
    q_stride,                                                                                 \
    kv_block_stride,                                                                          \
    kv_head_stride, exp_sums_ptr, max_logits_ptr, tmp_out_ptr,max_ctx_blocks);                                                                         

template<typename T, int BLOCK_SIZE=16, int HEAD_SIZE=128, int BLOCK_RATIO_PER_WG=4, int HEAD_ELEMS_PER_WG=4, int MAX_CONTEXT_PER_THREAD=4>
void paged_attention_custom_launcher(
  ...
  dim3 grid(num_seqs,max_num_partitions,num_heads/GQA_RATIO);
  dim3 block(NTHR);
  ...
  dim3 reduce_block(head_size); //TODO: assumes max_partitions < head_SIZE
  int reduce_shared_mem_size = 2 * max_num_partitions * sizeof(float);
  paged_attention_v2_reduce_kernel_new<T, HEAD_SIZE, HEAD_SIZE, NTHR>
