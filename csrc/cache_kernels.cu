...
void swap_blocks(
  ...
  for (const auto& pair : block_mapping) {
    ...
    cudaMemcpyAsync(
      reinterpret_cast<int64_t *>(dst_ptr) + dst_offset,
      reinterpret_cast<int64_t *>(src_ptr) + src_offset,
      block_size_in_bytes,
      memcpy_type,
      stream);
  }
...
__global__ void gather_cached_kv_kernel(
  ...
  const int x) {
    ...
    for (int i = threadIdx.x; i < num_tokens; i += blockDim.x) {
      ...
      key[tgt_key_idx] = key_cache[src_key_idx];
      value[tgt_value_idx] = value_cache[src_value_idx];
    }
}
    ...
    const int x)
{
    ...
    for (int i = threadIdx.x; i < unrolled_dim; i += blockDim.x)
    {
        ...
        for (int j = 0; j < unroll_factor; ++j)
        {
            ...
            keys_to_store[j] = key_cache[src_key_idx];
            values_to_store[j] = value_cache[src_value_idx];
        }
