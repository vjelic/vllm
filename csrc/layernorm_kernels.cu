...
__global__ void rms_norm_kernel(
    ...
    const float epsilon, const int num_tokens, const int hidden_size, const int vec_hidden_size) {
  ...
  vec8_t<scalar_t> const* vectorized_in =
      reinterpret_cast<vec8_t<scalar_t> const*>(input + bx * hidden_size);
  ...
  for (int idx = tx; idx < vec_hidden_size; idx += num_threads) {
    vec8_t<scalar_t> x = vectorized_in[idx];
    v8_variance += x * x;
  }
  ...
  float variance = rsqrtf(s_variance / hidden_size + epsilon);

  for (int idx = tx; idx < vec_hidden_size; idx += num_threads) {
    vec8_t<scalar_t> v8_in = vectorized_in[idx];
    vec8_t<scalar_t> v8_w = vectorized_weight[idx];
    vectorized_out[bx * vec_hidden_size + idx] =
        v8_in * variance * v8_w;
  }
}
...
void rms_norm(torch::Tensor& out,     // [..., hidden_size]
  ...
  VLLM_DISPATCH_FLOATING_TYPES(input.scalar_type(), "rms_norm_kernel", [&] {
    vllm::rms_norm_kernel<scalar_t><<<grid, block, 0, stream>>>(
        ...
        weight.data_ptr<scalar_t>(), epsilon, num_tokens, hidden_size, vec_hidden_size);
  });
}
