...
for bs,q_head,kv_head in configs:
    outputs = subprocess.run(f"python ./benchmarks/kernels/benchmark_paged_attention.py --context-len 8192 --block-size 16 --batch-size {bs} --head-size 128 --num-query-heads {q_head} --num-kv-heads {kv_head} --custom-paged-attn", shell=True, capture_output=True)
    print (outputs)
