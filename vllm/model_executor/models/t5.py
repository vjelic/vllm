    ...
    def __init__(self, config: T5Config, 
      quant_config: Optional[QuantizationConfig] = None):
        ...
        self.wi = ColumnParallelLinear(config.d_model, 
          config.d_ff, bias=False, quant_config=quant_config)
        self.wo = RowParallelLinear(config.d_ff, config.d_model, 
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, 
      ...
      has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = T5Attention(config, cache_config, 
          quant_config, 
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config, 
      ...
      quant_config: Optional[QuantizationConfig] = None):
        super().__init__()
        self.EncDecAttention = T5Attention(config, cache_config, 
          quant_config, has_relative_attention_bias=False)
        self.layer_norm = T5LayerNorm(config.d_model, 
          eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Block(nn.Module):
    def __init__(self, config: T5Config, 
      ...
      has_relative_attention_bias=False):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.self_attn = T5LayerSelfAttention(config, cache_config, 
          quant_config, 
          has_relative_attention_bias=has_relative_attention_bias)
        if self.is_decoder:
            self.cross_attn = T5LayerCrossAttention(config, 
    ...
    def forward(
        ...
        self_attention_outputs = self.self_attn(hidden_states, 
        ...
        do_cross_attention = (self.is_decoder and 
          encoder_hidden_states is not None)
        if do_cross_attention:
            cross_attention_outputs = self.cross_attn(hidden, kv_cache, 
              attn_metadata, encoder_hidden_states)
...
class T5Stack(nn.Module):
    def __init__(self, 
        ...
        self.block = nn.ModuleList(
            [T5Block(config, cache_config, quant_config, 
...
class T5Model(nn.Module):
    ...
    def __init__(self, 
        ...
        self.encoder = T5Stack(encoder_config, 
          cache_config, quant_config, self.shared)
