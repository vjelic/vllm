...
def _paged_attn_decode_v1_wo_dot_kernel(
    ...
    for blk_idx in range(num_ctx_blocks):
        ...
        if alibi_slopes is not None:
            _qk_0 += (alibi_slope *((blk_idx) * KV_BLK_SZ + blk_offs - seq_len + 1)).to(tl.float32)
        _qk_0 = tl.where(blk_idx*KV_BLK_SZ + blk_offs < seq_len, _qk_0, float("-inf")) 
        #Find max
...
def _paged_attn_decode_v1_w_dot_kernel(
    ...
    for b in range(num_kv_blks):
        ...
        if alibi_slopes is not None:
            qk += alibi_slope[:, None] * (b * KV_BLK_SZ + blk_offs - ctx_len + 1)[None, :]
    ...
    out_mask = (q_grp_offs[:, None] < QUERY_GRP_SZ) & (head_offs[None, :] < HEAD_SZ)
...
def paged_attn_decode_v2(
    ...
    #Note: There is a bug in next_power_of_2 function which causes it
