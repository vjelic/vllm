...
def torch_moe(a, w1, w2, score, topk):
    ...
    for i in range(w1.shape[0]):
        mask = topk_ids == i
        if mask.sum():
            # print("shape1",SiluAndMul()(a[mask] @ w1[i].transpose(0, 1)).shape, w2[i].transpose(0, 1).shape)
            # print("shape2",torch.nn.functional.gelu(a[mask] @ w1[i].transpose(0, 1)).shape, w2[i].transpose(0, 1).shape)
            out[mask] = torch.nn.functional.gelu(
