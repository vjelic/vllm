...
def run_rotary_embedding_neox(
    ...
    # Run the reference implementation with mods.
    #ref_rotary_embedding2 = RefRotaryEmbeddingNeox(
    #    dim=rotary_dim,
    #    max_position_embeddings=max_position,
    #    base=base,
    #).to(dtype=dtype, device='cuda')
    #out_query, out_key = ref_rotary_embedding(
    #    positions,
    #    query.view(num_tokens, -1, head_size),
    #    key.view(num_tokens, -1, head_size),
    #)
    #out_query = out_query.view(num_tokens, -1)
    #out_key = out_key.view(num_tokens, -1)

    # Compare the results.
    assert torch.allclose(out_query, ref_query, atol=4e-3, rtol=1e-5)
    assert torch.allclose(out_key, ref_key, atol=4e-3, rtol=1e-5)
