...
def run_timing(
    ...
    hidden_states = hidden_states.to(torch.float8_e4m3fnuz)
    ...
    a_scale = torch.randn(1, dtype=torch.float32, device=hidden_states.device)
    w_scale = torch.randn(1, dtype=torch.float32, device=hidden_states.device)
