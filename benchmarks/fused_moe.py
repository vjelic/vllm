...
def invoke_fused_moe_kernel(A: torch.Tensor, B: torch.Tensor, C: torch.Tensor,
    ...
    grid = lambda META: (triton.cdiv(em, block_size_m) * triton.cdiv(B.shape[1], META["BLOCK_SIZE_N"]), )

    # print(f"A_shape = {A.shape}")
    # print(f"EM = {sorted_token_ids.shape[0]}, N = {B.shape[1]}, K = {B.shape[2]}, num_tokens_post_padded = {num_tokens_post_padded}, topk_num = {topk_ids.numel()}")
...
def run_timing(
    ...
    for block_m in block_mn_range:
        ...
        ms1, min_ms1, max_ms1 = triton.testing.do_bench(lambda: invoke_fused_moe_kernel(hidden_states,
                                w1,
                                intermediate_cache1,
                                None,
                                None,
        ...
        ms2, min_ms2, max_ms2 = triton.testing.do_bench(lambda: invoke_fused_moe_kernel(intermediate_cache2,
                                w2,
                                intermediate_cache3,
                                None,
                                None,
