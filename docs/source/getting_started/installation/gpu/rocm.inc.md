...
To build vllm on ROCm 6.2 for Radeon RX7900 series (gfx1100), you should pick the alternative base image and specify `BUILD_FA` as below:

```console
DOCKER_BUILDKIT=1 docker build --build-arg BASE_IMAGE="rocm/vllm-dev:navi_base"  BUILD_FA="0"  -f Dockerfile.rocm -t vllm-rocm.
```
