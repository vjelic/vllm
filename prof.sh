...
#python benchmarks/benchmark_throughput.py --model $MODEL --input-len $input_len --output-len $gen_len --num-prompts $INP --tensor-parallel-size $tp
torchrun --standalone --nnodes=1 --nproc-per-node=$tp benchmarks/benchmark_prof.py --model $MODEL --input-len $input_len --output-len $gen_len --num-prompts $INP --tensor-parallel-size $tp --profile_rpd
